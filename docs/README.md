...
A locally-hosted web app for managing a multi-brand 3D printer farm. Replaces manual USB job distribution with centralized status monitoring and automated job dispatch. Supports Prusa (PrusaLink), Elegoo Centauri (SDCP), Bambu (MQTT), Klipper (Moonraker), OctoPrint, and Creality (K1/Ender series) printers.
...
| 6E | Complete | Creality driver: K1-platform (K1/K1C/K1 Max/Ender-3 V3/Hi) via Creality's local WebSocket (:9999) + HTTP upload API; K2 platform not supported |
