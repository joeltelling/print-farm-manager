...
- **State mapping (confirmed against real K1 telemetry).** Live status comes from **`deviceState`** (`0` = idle, non-zero = busy: printing / heating / leveling / self-test). `state === 5` → `PAUSED`. `deviceState` busy is checked **before** `err.errcode`, so a non-fatal hardware warning (e.g. mainboard fan) reported mid-print stays `PRINTING` — the code is only logged for diagnostics. If `deviceState` hasn't merged into the cache yet (e.g. the first frame after (re)connect carries only a latched `state`), the driver reports `UNKNOWN` rather than assuming idle — treating an unconfirmed `deviceState` as idle would let a stale `state 2`/`4` latch report `FINISHED`/`STOPPED` (and get credited) while the printer is still genuinely busy. `err.errcode ≠ 0` maps to `ERROR` only once `deviceState` is confirmed `0` (i.e. an error code that actually stopped the print, not a warning during a running one). Progress is `printProgress` (`dProgress` fallback), remaining time is `printLeftTime` (seconds), current file is `printFileName` reduced to a `path.basename` (the K1 reports a full on-printer path) with any timestamp prefix stripped. **Do not trust `state`, `printProgress`, or `printFileName` as live signals:** on real firmware `printProgress`/`printFileName` are *last-print residuals* (they stay at `100`/the old filename after a print), and `state` is a *phase/outcome latch* that reads `0`/`1` while running then latches `2` (completed) / `4` (stopped) while the device sits idle. `printId` is empty even mid-print and is unusable.
- **FINISHED / STOPPED, once, and only from an observed print.** Because `state` latches its outcome while idle, reporting it straight through would never return the printer to `IDLE`. The driver instead emits the terminal outcome (`FINISHED` from `state 2`, `STOPPED` from `state 4`) **exactly once** on the `deviceState` busy→idle edge — held in a per-connection `finishReported` flag, reset when the device is busy again — then reports `IDLE`. Crediting fires on the single `poller.js` transition and the printer returns to service. A held printer that reports `IDLE` still shows the operator confirmation buttons (`Fleet.jsx`), so sign-off is unaffected. The terminal outcome is additionally gated on a per-connection `sawBusy` flag: it is emitted only if this connection observed a busy `deviceState` (or a pause) since it (re)connected, and the flag clears when the socket closes. Without that gate, the first idle snapshot after a (re)connect would replay the previous print's latched `2`/`4` as a fresh event, and a reconnect while a database job was still `printing` would credit a job the driver never watched run. A print that completes while the driver is disconnected therefore surfaces through the poller's `PRINTING` to `IDLE` missed-finish hold (operator sign-off), never as a synthesized `FINISHED`.
