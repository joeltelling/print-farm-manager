...
| **Creality** | Creality local API (WebSocket push, port 9999) + HTTP multipart upload | K1, K1C, K1 Max, Ender-3 V3, Hi series (stock firmware). K2-platform printers (K2 Plus etc.) are NOT supported: they use a different on-printer G-code path (`/mnt/UDISK/printer_data/gcodes`) and a distinct CFS start flow this driver does not implement. |

The `printer.type` DB column stores the connector identifier: `prusa`, `elegoo-centauri`, `elegoo-centauri2`, `bambu`, `klipper`, `octoprint`, or `creality`. The `model` column is NOT cosmetic: it is the G-code routing key. The scheduler only dispatches a G-code whose `printer_model` equals the printer's `model` (`gcodes.printer_model = printers.model`), so editing a printer's model changes which files can be sent to that physical machine. Every printer write path (add, edit, CSV import) validates that the model's registered `connector` matches the printer's `type`, so a model cannot route another connector's G-code through the wrong driver.
...
- **Upload + print.** `POST http://<ip>/upload/<url-encoded-name>` as multipart (`file` field). The filename is sanitized first, spaces to underscores, exactly as CrealityPrint's `safe_filename()` does, and that one sanitized name is used for the upload URL, the multipart filename, the start command, and the confirmation check; the firmware stores the file under the sanitized name, so mixing raw and sanitized names makes a dispatch wait on a file that does not exist. Raw-body uploads make some firmware prepend the HTTP headers into the saved G-code (OrcaSlicer issue #8128), so multipart is required. The print is then triggered over the WebSocket with `{"method":"set","params":{"opGcodeFile":"printprt:/usr/data/printer_data/gcodes/<name>"}}`. `uploadAndPrint` resolves only on command-correlated confirmation: a print-phase status (`PRINTING`/`PAUSED`) **and** the reported filename matching the file this command just sent, polled from the cache for up to 20s, so unrelated device activity or a different print cannot confirm the dispatched job; a 409 raises `UPLOAD_CONFLICT`. The LAN API is unauthenticated by default; an optional `api_key`, if set, is sent as `Authorization: Bearer`.
