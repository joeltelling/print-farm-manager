...
Rather than converting the length figure to weight locally, which would need the filament's diameter and density (this app stores neither anywhere: `filament_types`/`filament_colors` only carry a name and a hex color), `reportJobUsage` now sends `use_length` to Spoolman when `filament_used_grams` is null but `filament_used_mm` is present. Spoolman already has the correct density on file for the bound spool's filament record, since that's the same value its own `use_length` handler uses to convert internally before writing the same `used_weight` column `use_weight` writes to (confirmed in `spoolman/database/spool.py`), so this defers the conversion to the system that actually has the data for it instead of guessing a density.

### Changes
- `server/integrations/spoolman.js`: `reportJobUsage` now reads both `filament_used_grams` and `filament_used_mm`, prefers weight, and falls back to sending `use_length` when only length was parsed. `no-parsed-usage` now only fires when both are null.
- `server/tests/spoolman-client.test.js`: added coverage for the `use_length` fallback payload, and for weight being preferred when both fields are present.
- `docs/spoolman.md`: documented the weight-preferred/length-fallback order and why a local mm-to-grams conversion is deliberately not done.

Hardware-validation honesty unchanged from the original usage-tracking chunk: implemented from Spoolman's API source (`Donkie/Spoolman`, master), not yet validated against a live Spoolman instance.
...
**Two different places to look, depending on source format.** A plain `.gcode` or a Bambu `.3mf`'s extracted plate G-code carries this as ordinary comments in the text itself (`; filament used [g] = 0.75`, `; total filament used [g] = 0.75`), so `parseFilamentUsage()` (`server/gcode-decode.js`) scans the already-decoded text directly. Prusa's binary `.bgcode` format is different: verified empirically against this repo's own real PrusaSlicer fixture, its GCode-type block (the only one `decodeBgcode` reads) contains zero occurrences of "filament used", the data lives entirely in separate Printer Metadata and Print Metadata blocks (types 3 and 4 per the [libbgcode spec](https://github.com/prusa3d/libbgcode/blob/main/doc/specifications.md)), as INI-style `key=value` lines with no leading `;` and no spaces around `=`. A new `extractBgcodeMetadataText()` reads those blocks specifically (`decodeBgcode` itself is unchanged and still only reads GCode blocks), and `parseFilamentUsage()`'s regex was made tolerant of both comment shapes so the same parser handles either source. Since this farm's whole printer fleet is Prusa (MK4S/MK4/Core One/XL), getting the `.bgcode` path right was the important case, not an edge case.

**Parsing precedence.** Prefers the last-occurring `total filament used [g|mm]` line (real slicer/bgcode output only ever writes this after every tool/object/wipe-tower's usage has been tallied, making it the authoritative aggregate for multi-tool/multi-color prints), and falls back to summing a non-"total" `filament used` line's comma-separated per-tool values when no total line exists. A regression test guards against the adjacent `total filament used for wipe tower [g]` line (present in real output right next to the real total) being mistaken for it. Only verified against real PrusaSlicer/OrcaSlicer/bgcode output; Bambu Studio's native filament-usage key is shaped differently (not a `[unit] = N` bracket-and-number line) and is not handled, flagging this rather than silently overclaiming Bambu support.

**A real decompression bug found and fixed along the way.** Reading the Print Metadata block against the real fixture failed under the existing Deflate decompression path: the block turned out to be zlib-wrapped (RFC 1950, a 2-byte header), not headerless raw Deflate (RFC 1951), which is all `decompressBlock` had ever assumed for the `Deflate` compression code (untested until now because the app's only real bgcode fixture uses Heatshrink, not Deflate, for its one previously-read block type, GCode). `decompressBlock`'s Deflate case now detects the zlib header (a checksum built into the format itself, essentially unmistakable) and picks the right `pako.Inflate` mode automatically; this benefits the existing GCode-block decode path too; a raw-deflate synthetic bomb test built with Node's own `zlib.deflateRawSync` (no wrapper) continues to pass unchanged.

**Lazy, non-destructive backfill.** Hooked into `GET /api/gcodes/:id/preview` rather than the list route, since it's the only place a full decode already happens on every request, so parsing the result for metadata is free by comparison, mirroring the existing `file_size` backfill's shape (self-heals on first real access, no startup migration loop). `filament_used_grams`/`filament_used_mm` (new nullable `gcodes` columns) are set only if still `NULL`, never re-parsed or overwritten afterward. `material_grams` is also backfilled from the same parse, but only when it was still `NULL`; a value already present, whether filename-derived or manually edited, always wins. The parsed values are reported via `X-Filament-Used-Grams` / `X-Filament-Used-Mm` response headers on the (still plain-text) `/preview` response, present only when found, so the existing `res.text()` / Web Worker contract driving the 3D render is untouched.

**Also surfaced in the Details panel, not just the 3D Viewer.** The `/preview` hook above only fires when the 3D Viewer is actually opened, so a G-code's filament data stayed invisible in the Details panel (the more commonly visited surface) until then. The existing per-gcode "Parse filename" button and its `POST /api/gcodes/parse-filename` endpoint only ever read the filename string, real content was sitting right there on disk, unread. That button is now "Parse G-code", backed by a new `POST /api/gcodes/:id/parse-gcode` endpoint that decodes the already-uploaded file fresh on every click and returns real print time (a new `parsePrintTime()` in `server/gcode-decode.js`, reading the slicer's own "estimated printing time (normal mode)" line) and filament weight, populating the same draft inputs the old filename parse used to, so nothing in that flow regresses, it just reads better data. As a side effect it also persists `filament_used_grams`/`filament_used_mm` (the same non-destructive `COALESCE` rule), so the Details panel's row now shows a persistent "Filament used" line once known, whether that happened via this button or a prior 3D Viewer open. `POST /api/gcodes/parse-filename` itself is untouched and still backs the upload form's automatic pre-fill, since no file has been uploaded yet at that point and filename is genuinely the only signal available.

### Changes
- `server/gcode-decode.js`: new `parseFilamentUsage()`, `parsePrintTime()`, `extractBgcodeMetadataText()`, and a refactored `parseBgcodeBlocks()` shared by `decodeBgcode()` and `extractBgcodeMetadataText()`; `decompressBlock`'s Deflate case now auto-detects zlib-wrapped vs. raw payloads.
- `server/db.js`: additive `gcodes.filament_used_grams`, `gcodes.filament_used_mm` columns.
- `server/routes/gcodes.js`: `GET /:id/preview` lazily parses and persists filament-usage stats (and backfills `material_grams`), reporting them via new response headers; new `POST /:id/parse-gcode` re-derives print time and filament weight from real file content, backing the repurposed "Parse G-code" button.
- `client/src/GcodeViewerModal.jsx`: reads the new headers and renders a filament-used line under the modal title when present.
- `client/src/pages/Projects.jsx`: `GcodeEstimateRow`'s "Parse filename" button and handler are now "Parse G-code", calling the new endpoint; renders a persistent "Filament used" line sourced from `filament_used_grams`/`filament_used_mm`.
- `server/tests/gcode-decode.test.js`, `server/tests/gcodes.test.js`, `server/tests/backup-restore.test.js`: parser unit tests (including live regressions against the real fixture for both filament and print-time parsing), route-level tests for both endpoints, and the backup/restore column round-trip.
- `docs/database.md`, `docs/api.md`, `docs/web-app.md`, `docs/server.md`: documented the new columns, endpoints, response headers, and both display locations.

Verified: full suite passes (475 tests), including the live fixture regressions (`extractBgcodeMetadataText` + `parseFilamentUsage`/`parsePrintTime` against the real `mini_cube_ps2.8.1.bgcode` fixture yield the fixture's actual known values, 0.75g / 252.22mm / 221s) and the new zlib-wrapped-Deflate decompression test. `decodeBgcode`'s own pre-existing byte-for-byte real-fixture regression test (every G0-G3 move line matching the official reference decode) still passes unchanged, confirming the `parseBgcodeBlocks()` refactor didn't alter its behavior. Also verified live against the running dev server and real uploaded files (`.gcode`, `.bgcode`, `.3mf`, one of which is this repo's own real PrusaSlicer fixture): opened the 3D Viewer on each and confirmed the correct "Filament used" line and a rendering toolpath with no console errors; confirmed a `.3mf` with no embedded plate G-code still fails with its pre-existing error message, unaffected by this change.
