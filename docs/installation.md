...
**`.local` (mDNS) names**, such as `octoprint.local`, are a special case: your OS resolves them natively (Windows, macOS), but the published Docker image has no OS-level mDNS resolver, so a `.local` name that works fine typed into a browser on the host would previously time out from inside the container. The app resolves `.local` names itself now (`server/mdns-resolve.js`, an mDNS query sent directly over UDP multicast, independent of the OS resolver), so this works the same in Docker as anywhere else, and the printer/settings form no longer needs to warn you away from it. This does depend on mDNS multicast traffic actually reaching the container: Docker's default bridge network usually allows it, but some network setups (restrictive host firewalls, certain overlay/VPN configurations) block multicast, in which case a `.local` name falls back to showing OFFLINE exactly as before. If that happens, using the printer's IP address instead (ideally with a DHCP reservation, since it bypasses the question entirely) remains the reliable option.
