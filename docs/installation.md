...
- **`.local` (mDNS) names** rely on your OS resolving them, which Windows and macOS do natively but the published Docker image does not (no mDNS resolver in the container). This is the single most common cause of a printer that shows OFFLINE in the app while the same `.local` name works fine when you type it into a browser on your own machine: your browser resolves it, the container cannot. The printer/settings form flags this with an inline warning as soon as you type a `.local` name. A plain DNS hostname works in Docker; a bare `.local` name does not. Until the printer's actual network address is known to resolve without mDNS, use its IP address instead, ideally with a DHCP reservation on your router so the address does not drift after a reboot.
