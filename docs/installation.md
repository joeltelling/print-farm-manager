...
Every "IP address" field above also accepts a hostname (`octoprint.lan`, `voron1.printfarm.internal`) instead of a numeric address: the app just uses it as-is in each request, so any name your network can resolve works. One caveat:
- **Elegoo Centauri Carbon** (the original, not Carbon 2) discovers itself over a UDP broadcast that only works with an IPv4 address, so a hostname there must resolve to an IPv4 address (true of virtually any normal DNS name, and always true of a `.local` name once resolved, since mDNS A records are IPv4).

**`.local` (mDNS) names**, such as `octoprint.local`, are a special case: your OS resolves them natively (Windows, macOS), but the published Docker image has no OS-level mDNS resolver, so a `.local` name that works fine typed into a browser on the host would previously time out from inside the container. The app resolves `.local` names itself now (`server/mdns-resolve.js`, an mDNS query sent directly over UDP multicast, independent of the OS resolver), so this works the same in Docker as anywhere else, and the printer/settings form no longer needs to warn you away from it. This does depend on mDNS multicast traffic actually reaching the container: Docker's default bridge network usually allows some of it, but forwards it unreliably in practice, which can look like a `.local` printer connecting sometimes and not others, or a different subset of `.local` printers succeeding between one poll and the next, rather than a clean pass or fail. If that happens, two options:

- **Switch the container to host networking.** Add `network_mode: host` to the `print-farm-manager` service in `docker-compose.yml` and remove its `ports:` section (Compose ignores it under host networking; the app still listens on port 3000, just directly on the host's real network interface instead of through Docker's bridge, which is the standard fix for unreliable multicast in a container). The tradeoff: the container no longer has its own address on Docker's internal network, so a reverse proxy on the same Docker host that reached it by service name (e.g. `http://print-farm-manager:3000`) needs to point at the host's own IP address instead (or `host.docker.internal`, where the proxy's engine provides it).
- **Use the printer's IP address instead** (ideally with a DHCP reservation, since it bypasses the question entirely). Zero configuration change, and remains the reliable fallback for any printer whose `.local` name doesn't resolve consistently even with host networking.
