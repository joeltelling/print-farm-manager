...
| **Creality (K1 / Ender-3 V3 / Hi)** | IP address (API key optional) | Printer touchscreen: **Settings → Network** shows the IP. Most K1-series printers on the LAN need no API key, so leave it blank. Only fill it in if you've configured one (e.g. Creality Print / Nebula). Port 9999 is used automatically. K2-platform printers (K2 Plus etc.) are not supported by this connector. |
